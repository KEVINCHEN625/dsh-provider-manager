...
- **Custom API presets.** One card each for **ZCode**, **MiMo**, and **MiniMax** with **China / Overseas** URLs. OpenRouter, SiliconFlow, Moonshot, DeepSeek, OpenAI, Anthropic, Groq, Together, Fireworks, DashScope, Gemini, Mistral, and one-click **Meta Model API**. Meta is **pay-as-you-go**, not a Muse Code CLI subscription.
...
**Custom API** — `llm-pi-ai` route `^[a-z][a-z0-9-]*$`. **Add provider** can start from a known API. ZCode, MiMo, MiniMax, SiliconFlow, and Moonshot stay on **one card**: choose **China** or **Overseas**, then save. The key must match the console that issued it.
